...
class GasTempDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scale = self.data.iloc[idx]['maxs']-self.data.iloc[idx]['mins']
        temp_adjust = (math.cos(shift)+1)*scale/2 + self.data.iloc[idx]['mins']
        model_temp = self.data.iloc[idx]['avg_temp']-temp_adjust
